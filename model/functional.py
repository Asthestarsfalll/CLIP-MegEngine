...
def multi_head_attention(
    ...
    if in_proj_weight is not None:
        query, key, value = split(linear(query, in_proj_weight, in_proj_bias, compute_mode), 3, axis=-1)
    else:
...
class MultiheadAttention(M.Module):
    ...
    def __init__(
        ...
        if add_bias_kv:
            self.bias_k = M.Parameter(mge.random.normal(size=(1, 1, embed_dim)))
            self.bias_v = M.Parameter(mge.random.normal(size=(1, 1, embed_dim)))
        else:
