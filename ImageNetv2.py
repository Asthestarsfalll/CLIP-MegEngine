...
from megengine.data.dataset.vision import ImageFolder
...
class ImageNetV2Dataset(Dataset):
    ...
    def __getitem__(self, i):
        print(type(self.fnames[i].__str__()), self.fnames[i].__str__())
        img, label = cv.imread(self.fnames[i].__str__(), 1), int(self.fnames[i].parent.name) 
